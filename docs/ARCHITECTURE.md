...
| --- | --- | --- | --- |
| 同一用户目录的应用进程 | Electron 单实例锁 | 获得锁的主进程 | 窗口激活与全部本机写入者 |
...
| 当前选台、线路轮换、断网恢复与健康观察 | `PlaybackController` 的当前播放代次 | `PlaybackController` | 界面回调、`StreamPlayer`、`ViewingState` |
...
渲染入口保留界面与目录流程的组装，播放策略由阶段七引入的 `PlaybackController` 负责；`channel-list.ts` 管理筛选与虚拟列表，`keyboard-controls.ts` 管理快捷键和数字选台，`catalog-status-view.ts` 只展示目录状态。频道查找使用当前目录的索引；索引是目录投影，不是第二份持久化权威。滚动事件在下一帧合并，可视范围未变时不重建行；选择、收藏、目录或台标变化仍强制更新。
...
## 阶段七：单写者、提交确认、依赖检查与工作生命周期

本阶段的逐项清单与验收结果见 [架构优化清单](ARCHITECTURE_OPTIMIZATIONS.md)。

### 单实例与安全状态提交

- 设置用户目录后、创建任何服务或窗口前取得 `app.requestSingleInstanceLock()`；同目录的新进程立即退出，已有窗口恢复并激活。在 macOS 关闭窗口但应用仍在后台时，再次启动会重建窗口。不同用户目录仍可独立运行。
- 单实例锁使已有进程内串行写入队列继续成立，不增加跨进程文件锁、数据库或多写者协议。
- 安全文件替换前的失败保留上次已确认状态；替换成功后的复读失败由 `SafetyStateCommitUncertainError` 明确标识，不能被当成“未写入”。
- `SafetyCoordinator` 收到提交不确定错误后，先阻断目录和远程资源准入、取消台标及播放，再尝试读取正式文件。只有所有状态字段与本次预期提交一致时才更新内存并恢复服务。
- 短暂读回故障可在同次操作内恢复；持续故障保持阻断，下次初始化或设置操作会重试。重启仍读取同一个安全文件，并沿用待清理标记完成恢复。

### 依赖边界与类型环境

模块依赖检查使用 `@babel/parser` 解析实际导入、重导出、类型导入、动态导入和 `require`，解析后的相对路径先规范化再按层检查。裸 Node 模块、深层目录和副作用导入不能绕过边界；渲染层也不能经共享模块间接执行目录安全投影。变量动态导入和未声明的包或路径别名默认拒绝；唯一动态路径例外仍是主入口的显式冒烟驱动。

`tsconfig.main.json`、`tsconfig.preload.json`、`tsconfig.renderer.json`、`tsconfig.shared.json` 分别检查四层。Node/Electron 类型只进入前两层，渲染和共享层不能直接使用 `process`、`Buffer` 等 Node 全局变量。根配置只负责测试与构建配置；负向测试会在真实渲染和共享源码参与的类型环境中验证隔离。CSP、IPC 和网络规则继续承担运行时约束。

### 目录辅助信息与取消

- 频道、线路和 blocklist 必须成功且仍执行每次最多 3 次、每次 30 秒的获取策略；不能用辅助信息的降级路径放宽安全输入。
- 国家、分类和台标元数据共用 5 秒等待预算，单次请求最多 2.5 秒，最多尝试 2 次。预算从核心数据完成后开始计时，因此只限定辅助阶段，不承诺整个同步在 5 秒内完成。失败时返回缺少对应辅助信息的真实目录并给出提示。
- 每个协调器有最多三项、12 小时有效的辅助信息内存缓存；较早操作不能覆盖较新结果。缓存不持久化，不包含安全过滤输入，也不保存台标图片。清除磁盘目录时仍可保留本次运行中的辅助信息；重启后重新获取。
- 每个目录调用者独立订阅任务；新请求先订阅可合并的任务，再撤销同窗口旧请求。单个调用者取消不影响其他等待者，全部退出后中止网络请求与重试等待。
- 页面导航、渲染进程退出、家庭模式切换、进入演示及应用退出会撤销相应调用者或任务。取消后的迟到结果不能被接受，尚未开始的缓存写入被拒绝；已经进入原子写入的工作仍按原有队列和缓存纪元完成或清除。

### 播放协调与兼容

`PlaybackController` 统一拥有选台、当前线路、失败线路集合、重连代次、恢复定时器和每次播放的健康观察。界面入口只传入用户动作并更新显示；`StreamPlayer` 继续拥有视频元素、HLS、媒体指标和播放会话释放，`ViewingState` 继续是观看数据的唯一持久化写入者。

控制器可注入网络检查、时钟和界面回调，测试直接验证停止和切台后的迟到结果失效、重复在线事件只重连一次、断网不污染线路健康、失败线路耗尽后的手动重试，以及稳定播放只记录一次成功。

本阶段不改变安全状态 V1、目录缓存 V2、观看记录键或公共 IPC 格式；不清空既有用户资料，也不增加双写或兼容开关。回退旧代码会失去本阶段的单实例与提交确认保证，因此不得让新旧版本同时使用同一个用户目录。
...
`verify:single-instance` 用隔离目录启动真实生产应用，验证重复进程退出、最小化窗口恢复、安全设置不被覆盖、独立目录可同时运行，以及 macOS 后台应用重开已关闭窗口。Linux 与 macOS CI 都执行此项。
