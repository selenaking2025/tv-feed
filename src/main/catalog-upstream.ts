...
export async function fetchIptvOrgBundle(
  ...
  const [channels, streams] = await Promise.all([
    fetchRequired<UpstreamChannel>('channels', '频道', CORE_ATTEMPTS, report, fetcher, delay),
    fetchRequired<UpstreamStream>('streams', '播放线路', CORE_ATTEMPTS, report, fetcher, delay)
  ])
  const blocklist = await fetchRequired<UpstreamBlocklistEntry>(
    'blocklist', '安全过滤', CORE_ATTEMPTS, report, fetcher, delay
  )
  ...
  const [countriesResult, categoriesResult] = await Promise.all([
    fetchOptional<UpstreamCountry>('countries', '国家信息', OPTIONAL_ATTEMPTS, report, fetcher, delay),
    fetchOptional<UpstreamCategory>('categories', '分类信息', OPTIONAL_ATTEMPTS, report, fetcher, delay)
  ])
  const logosResult = await fetchOptional<UpstreamLogo>(
    'logos', '台标信息', OPTIONAL_ATTEMPTS, report, fetcher, delay
  )
...
async function fetchRequired<T>(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      return await fetchEndpoint<T>(endpoint, label, fetcher)
    } catch (error) {
...
async function fetchOptional<T>(
  ...
  fetcher: SecureFetcher,
  delay: (milliseconds: number) => Promise<void>
): Promise<{ records: T[]; warning: string }> {
  try {
    return { records: await fetchRequired<T>(endpoint, label, maxAttempts, report, fetcher, delay), warning: '' }
  } catch {
